# -----------------------------------------------------------------------------
# 파일명 : log_gateway/runtine_pipeline.py
# 목적   : generator의 큐 기반 파이프라인(서비스 배치 루프 + Kafka 발행 워커) 구성
# 설명   : 서비스별 목표 RPS를 유지하며 배치 생성 → publish 큐 → Kafka 전송으로 이어지는
#         파이프라인을 관리하고, generator.py가 더 간결하게 책임을 분리하도록 함
# -----------------------------------------------------------------------------
...
def _resolve_pipeline_settings(profile: Dict[str, Any]) -> Tuple[int, int, int, int]:
    """프로파일에서 배치/큐/워커 설정을 추출."""
    batch_cfg = profile.get("batch", {})
    batch_min = max(1, int(batch_cfg.get("min", 50)))
    batch_max = max(batch_min, int(batch_cfg.get("max", 200)))
    queue_size = max(0, int(profile.get("publish_queue_size", 10000)))
    publisher_workers = max(1, int(profile.get("publisher_workers", 4)))
    return batch_min, batch_max, queue_size, publisher_workers
...
def start_pipeline(
    ...
    """큐/워크플로 태스크를 초기화하고 반환."""
    batch_min, batch_max, queue_size, publisher_workers = _resolve_pipeline_settings(profile)
